...
# 做了两个东西：v和a
# 删除gail部分	:param gail: (bool) Whether we are using this generator for standard trpo or with gail
#		在gail下，rewards和true_rewards不一样；删除true_rewards,true_reward,cur_ep_true_ret,ep_true_rets
...
def traj_segment_generator( policy, env, horizon, reward_giver=None, callback=None ):
	step = 0  # 跨多次迭代此generator计和；
	current_it_len = 0  # len of current iteration， 当前的，即最后应该是horizon？
	...
	def learn( self, total_timesteps, callback=None, log_interval=100, tb_log_name="TRPO",
		...
		with SetVerbosity( self.verbose ), TensorboardWriter( self.graph, self.tensorboard_log, tb_log_name, new_tb_log ) \
			...
			with self.sess.as_default():
				...
				seg_gen = traj_segment_generator( self.policy_pi, self.env, self.timesteps_per_batch, reward_giver=self.reward_giver, callback=callback )
...
# 		callback.update_locals(locals())
# 		if callback.on_step() is False: # false指abort
